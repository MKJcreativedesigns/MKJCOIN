...
export default function SellCrypto() {
  ...
  const destinationOptions = [
    { id: 'bank', name: 'Bank Transfer (ACH)', fee: '0.1%', time: '1-3 days' },
    { id: 'card', name: 'Original Debit Card', fee: '1.5%', time: 'Instant' },
    { id: 'wallet', name: 'MKJ Wallet (USD Cash)', fee: 'Free', time: 'Instant' },
  ];
      ...
      <main className="pt-32 pb-20 px-4">
        <div className="max-w-xl mx-auto">
          ...
          >
            ...
            <h1 className="text-3xl font-heading font-bold mb-2 text-red-500">Sell Crypto</h1>
            <p className="text-white/50 mb-8">Convert your assets to cash instantly.</p>

            <div className="space-y-6 relative z-10">
              {/* Asset Selection */}
              <div>
                <label className="text-sm text-white/40 mb-2 block">I want to sell</label>
                  ...
                  {assets.map((a) => (
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all ${
                        asset === a.symbol ? 'bg-red-500/10 border-red-500 text-white' : 'bg-white/5 border-white/5 text-white/40 hover:bg-white/10'
                      }`}
                    >
                      {a.icon}
              ...
              <div className="bg-[#0B0E14]/60 border border-white/10 rounded-2xl p-6">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-sm text-white/40">Amount in {asset}</span>
                  <span className="text-xs text-red-400">Balance: 0.155 BTC</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-3xl font-bold">{asset}</span>
                  <input
                    type="number"
                    value={amount}
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-white/40">Estimated USD</span>
                  <span className="font-bold text-red-400">≈ ${(Number(amount) * 64320).toFixed(2)}</span>
                </div>
              </div>

              {/* Deposit Result */}
              <div>
                <label className="text-sm text-white/40 mb-2 block">Deposit to</label>
                  ...
                  {destinationOptions.map((o) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-4 h-4 rounded-full border border-white/20 flex items-center justify-center p-0.5 group-hover:border-red-500">
                          {o.id === 'bank' && <Check className="w-full h-full text-red-500" />}
                        </div>
                        <div>
                          <p className="font-semibold text-sm">{o.name}</p>
                          <p className="text-[10px] text-white/40">{o.time} • Fee: {o.fee}</p>
                        </div>
                      </div>
              ...
              <div className="flex items-center justify-center gap-2 text-xs text-white/30">
                ...
                Exchange rates are updated every 15 seconds.
              </div>
            </div>
          </motion.div>
        </div>
